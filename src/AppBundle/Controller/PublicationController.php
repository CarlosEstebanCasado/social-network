...
class PublicationController extends Controller{
		...
		if($form->isSubmitted()){
			if($form->isValid()){
				...
				if(!empty ($doc) && $doc != null){
					...
					if($ext == 'pdf'||$ext == 'xlsx'||$ext == 'xls'|| $ext == 'docx'|| $ext == 'doc'|| $ext == 'txt'|| $ext == 'odt'||$ext == 'csv'){
						$file_name = $user->getId().time().".".$ext;
	...
	public function removePublicationAction(Request $request, $id = null, $publication_like = null){
		$em = $this->getDoctrine()->getManager();
		//$em2 = $this->getDoctrine()->getManager();
		$publication_repo = $em->getRepository('BackendBundle:Publication');
		$likes_repo = $em->getRepository('BackendBundle:Like');
		$publication = $publication_repo->find($id);
		$like = $likes_repo->findBy(array('publication' => $publication_like));
		$user = $this->getUser();

		if($user->getId() == $publication->getUser()->getId()){
			$em->remove($publication,$publication_like);
			//$em2->remove($publication_like);
			$flush = $em->flush();

			if($flush == null){
				$status = 'La publicación y los likes se han borrado correctamente';
			}else{
				$status = 'La publicación y los likes no se han borrado';
			}
		}else{
			$status = 'La publicación y los likes no se ha borrado';
		}
